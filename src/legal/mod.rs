...
fn statute_expert(data: &CaseData, _config: &LegalMoeConfig, _db: Option<&sled::Db>) -> ExpertPrediction {
...
fn complaint_expert(data: &CaseData, _config: &LegalMoeConfig, db: Option<&sled::Db>) -> ExpertPrediction {
    ...
    let _real_violations: usize = real_complaints.iter().map(|c| c.violations_found.len()).sum();
